...
"""
NN4 trains a small neural network in which we assign a weight to each vector in the sentence
and take the mean. We learn a quadratic weight function through a quadraticWeightsDotMeanLayer.
"""
class NN4():
    def __init__(self):
        ...
        self.model1 = NN_layers.quadraticWeightsDotMeanLayer(self.numpy_rng, self.theano_rng, self.x1, INPUT_SHAPE,
                                                             self.l1, max_length=30, n_out=OUTPUT_SHAPE[1], batch_size=BATCH_SIZE, W=None)
        self.model2 = NN_layers.quadraticWeightsDotMeanLayer(self.numpy_rng, self.theano_rng, self.x2, INPUT_SHAPE,
                                                             self.l2, max_length=30, n_out=OUTPUT_SHAPE[1], batch_size=BATCH_SIZE, W=self.model1.W)
    ...
    def run(self, epochs=EPOCHS, learning_rate=LEARNING_RATE, regularization=REGULARIZATION, momentum=MOMENTUM):
        processor = NN_process.lengthPairProcessor('../data/pairs/sets/enwiki_pairs_' + WORDS_FILE + '-train.txt',
