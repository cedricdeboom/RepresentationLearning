...
"""
NN3 trains a small neural network in which we multiply idf with the word vector and a trained weight,
all through the doubleDotMeanLayer.
"""
...
class NN3():
    def __init__(self):
        ...
        self.model1 = NN_layers.doubleDotMeanLayer(self.numpy_rng, self.theano_rng, self.x1, self.x1_bis, INPUT_SHAPE, n_out=OUTPUT_SHAPE[1], W=None)
        self.model2 = NN_layers.doubleDotMeanLayer(self.numpy_rng, self.theano_rng, self.x2, self.x2_bis, INPUT_SHAPE, n_out=OUTPUT_SHAPE[1], W=self.model1.W)

        self.params = []
        self.params.extend(self.model1.params)

    def run(self, epochs=1, learning_rate=1.0, regularization=0.0, momentum=0.01):
        processor = NN_process.PairBisProcessor('../data/pairs/sets/enwiki_pairs_20-train.txt', '../data/pairs/sets/enwiki_no_pairs_20-train.txt',
        ...
        train_x1_bis = theano.shared(value=processor.x1_bis, name='train_x1_bis', borrow=False)
        train_x2_bis = theano.shared(value=processor.x2_bis, name='train_x2_bis', borrow=False)
        ...
        train = self.train_function_momentum(train_x1, train_x2, train_x1_bis, train_x2_bis, train_y, train_z)
    ...
    def train_function_momentum(self, x1, x2, x1_bis, x2_bis, y, z):
    ...
    def calculate_cost(self):
        ...
        weights = (self.model1.W ** 2).sum() + numpy.log(metrics.N_DOCUMENTS)
        loss = ((((output1 - output2) ** 2).sum(axis=1) / weights) - self.y) * (-self.z)
        #distance = (1.0 - (output1 * output2).sum(axis=1) / T.sqrt((output1 * output1).sum(axis=1)) / T.sqrt((output2 * output2).sum(axis=1))) / 2.0
