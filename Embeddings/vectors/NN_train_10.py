...
BATCH_SIZE = 100
EMBEDDING_DIM = 400
WORDS = 30  #i.e. max number of words
EPOCHS = 3
LEARNING_RATE = 1.0
MOMENTUM = 0.0
...
"""
NN10 trains a small neural network in which we assign a weight to each vector in the sentence, normalize and cutt off.
...
class NN10():
    def __init__(self):
        ...
        self.model1 = NN_layers.interpolatingDotMeanLayer(self.numpy_rng, self.theano_rng, self.x1, INPUT_SHAPE,
                                                             self.indices1, self.l1, max_length=35, n_out=OUTPUT_SHAPE[1], batch_size=BATCH_SIZE, W=None)
        self.model2 = NN_layers.interpolatingDotMeanLayer(self.numpy_rng, self.theano_rng, self.x2, INPUT_SHAPE,
